...
while True:    
    ...
    if(payout != payouting):
        payout = payouting
        ActionChains(driver).click(inputB[0]).key_down(Keys.CONTROL).send_keys("a").key_up(Keys.CONTROL).send_keys("100").perform()
        #inputB[0].clear()
        #inputB[0].send_keys(100)
        ActionChains(driver).click(inputB[1]).key_down(Keys.CONTROL).send_keys("a").key_up(Keys.CONTROL).send_keys("2").perform()
        #inputB[1].clear()
        #inputB[1].send_keys(2)
        betBut = payouting = driver.find_element_by_xpath("/html/body/div[1]/div/div[1]/div[2]/div/div[2]/div[2]/div/button/div").click()
        print (payout)
    else:
        time.sleep(0.3)
