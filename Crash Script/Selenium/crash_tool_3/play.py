...
def auto_run():
    ...
    donate_frame = Frame(root, relief=RAISED, borderwidth=1)
    donate_frame.pack(fill=X, padx=0, pady=0)

    donate_label = Label(donate_frame, text="Donate(TRX - USDT - BTT): ", width=25, fg = "green")
    donate_label.pack(side=LEFT, padx=0, pady=0)
    address_donate = StringVar(root, value='TJLKaDdAdgJG1nnHaPYEeTozw2qTUBTYp4')
    output_address_donate = Entry(donate_frame,textvariable=address_donate, width=50)
    output_address_donate.pack(side=LEFT, padx=2, pady=0)
                ...
                try:
                    ...
                    if(lastPayout != payout):
                        ...
                        with open("./data/history_dice.txt", "a+") as hd:
                            ...
                            if(getnext(historyValue) == False and 2 not in historyValue):
                                noBet = randint(0, 6)
                                bet = arrBet[noBet]
                                timeBet += 1
                                ...
                                if(timeRun != 1):
                                    ...
                                    if(check2):
                                        ...
                                        beting_label["text"] = "Beting: " + str(round(bet,4))
                                        textarea.insert(END,str(timeRun) + ".Beting: " + str(round(bet,4)) +"\n")
                                        while True:    
                                            ...
                                            if(payout2 != lastPayout):
                                                if(payouting <= payout2):
                                                    ...
                                                    textarea.insert(END,"Payout: " + str(payout2) +"\n")
                                                    if(payouting < 2):
                                                        bet = round(bet * (1/(payouting - 1) + payouting),4)
                                                    else:
                                                        bet = round(bet * (1 + 1/(payouting-1)),4)
                                                    if(bet >= maxBet):
                                                        maxBet = bet
                                                        hd.write("New max bet: %.4f\n" %maxBet)
                                                        textarea.insert(END,"New max bet: " + str(round(maxBet,4)) +"\n","lost")
                                                        bet_max_label["text"] = "New max bet: " + str(round(maxBet,4))
                                ...
                                time_label["text"] = "Time running: " + str(minutes) + " min"
