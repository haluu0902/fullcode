...
public class MainController {
    ...
    public MainController(LoadToTableEx3 f1) {
        this.f1 = f1;
    }
    ...
    private void initLstSearch() {
        f1.getLstSearchIn().setModel(new DefaultComboBoxModel<>(title));
        f1.getLstSearchIn().setSelectedIndex(0);
    }
...
//        dosth();
    ...
    private void Load(String sql) {
        ...
        try {
            ...
            if (n == 0) {
                f1.getLblDisplay().setText("Khong co ban ghi");
            } else {
                f1.getLblDisplay().setText("Da tim thay " + n + " ban ghi");
            }
        } catch (Exception e) {
    ...
    private void xulySearch() {
        f1.getBtnSearch().addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                int[] select = f1.getLstSearchIn().getSelectedIndices();
                ArrayList<Integer> ar = new ArrayList();

                int n = select.length;
                if (n == 0) {
                } else {
                    String sql = ss + " and (" + SearchField[select[0]] + " like '%" + f1.getTxtSearchIn().getText() + "%'";
                    if (n > 1) {
                        for (int i = 1; i < n; i++) {
                            sql = sql + " or " + SearchField[select[i]] + " like '%" + f1.getTxtSearchIn().getText() + "%'";
                        }
                    }
                    sql = sql + ")";
...
//    }
    ...
    public static void main(String[] args) {
        new MainController(new LoadToTableEx3()).control();
    }
