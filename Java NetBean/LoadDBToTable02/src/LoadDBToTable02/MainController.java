...
//        String sql = "Select * from StudentInfor";
        String sql = "Select s.ID, s.Name, s.Gender, d.DepartName,s.GPA, s.[Address] from StudentInfor s, DepartInfor d where s.DepartID=d.DepartID;";
        dftm.setRowCount(0);
        try{
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
      ...
      private void xulyUpdate(){
          f1.getBtnUpdate().addActionListener(new ActionListener() {
              @Override
              public void actionPerformed(ActionEvent e) {
                  String sql = "update StudentInfor set Name= ?,Gender=?"+",DepartID=? ,GPA=?,Address=? where ID =?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
      ...
      private void xulyDelete(){
        ...
        if(ans==0){
        String sql = "delete from StudentInfor where ID =? ";
            try{
                PreparedStatement ps = con.prepareStatement(sql);
                ps.setString(1,f1.getTxtID().getText());
                ps.execute();
